...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tca_moneda_referencia_name");
			if (d != null)
			{
		...
		d = rs.getString("tca_fecha_vigencia_ini");
			if (d != null)
			{
		...
		d = rs.getString("tca_moneda_referencia");
			if (d != null)
			{
			name  = "b_referenced_coin";
			label = "${lbl:b_referenced_coin}";
		...
		d = rs.getString("smn_monedas_id_name");
			if (d != null)
			{
			name  = "b_coin";
			label = "${lbl:b_coin}";
		...
		d = rs.getString("tca_fecha_vigencia_fin");
			if (d != null)
			{
			name  = "b_validation_date_until";
			label = "${lbl:b_validation_date_until}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

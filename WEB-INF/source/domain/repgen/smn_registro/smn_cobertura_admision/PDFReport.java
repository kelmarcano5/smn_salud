package domain.repgen.smn_registro.smn_cobertura_admision;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("coa_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("coa_estatus_financiero");
			if (d != null)
			{
			name  = "b_status_financiero";
			label = "${lbl:b_status_financiero}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("coa_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("coa_estatus_proceso");
			if (d != null)
			{
			name  = "b_status_proceso";
			label = "${lbl:b_status_proceso}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_presupuesto_id");
			if (d != null)
			{
			name  = "b_presupuesto";
			label = "${lbl:b_presupuesto}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

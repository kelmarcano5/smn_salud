package domain.repgen.smn_registro.smn_citas_mov;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("ctm_fecha_cita");
			if (d != null)
			{
			name  = "b_appointment_date";
			label = "${lbl:b_appointment_date}";
		...
		d = rs.getString("ctm_num_control_cita");
			if (d != null)
			{
			name  = "b_num_control_citas";
			label = "${lbl:b_num_control_citas}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ctm_fecha_registro");
			if (d != null)
			{
			name  = "{b_store_date}";

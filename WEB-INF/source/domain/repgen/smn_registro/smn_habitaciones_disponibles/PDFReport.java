package domain.repgen.smn_registro.smn_habitaciones_disponibles;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("had_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("had_fecha_egreso");
			if (d != null)
			{
			name  = "b_date_egr";
			label = "${lbl:b_date_egr}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("had_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("had_fecha_ingreso");
			if (d != null)
			{
			name  = "b_date_ing";
			label = "${lbl:b_date_ing}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

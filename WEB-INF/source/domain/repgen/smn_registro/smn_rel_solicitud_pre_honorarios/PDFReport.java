package domain.repgen.smn_registro.smn_rel_solicitud_pre_honorarios;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("rsh_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("rsh_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_solicitud_presupuesto_id");
			if (d != null)
			{
			name  = "b_budget_request_honorary";
			label = "${lbl:b_budget_request_honorary}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

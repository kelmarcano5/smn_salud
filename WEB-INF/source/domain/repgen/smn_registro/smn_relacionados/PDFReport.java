package domain.repgen.smn_registro.smn_relacionados;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_afiliados_id_name");
			if (d != null)
			{
			name  = "b_rel_afiliado";
			label = "${lbl:b_rel_afiliado}";
		...
		d = rs.getString("rel_numero_control");
			if (d != null)
			{
			name  = "b_rel_num_control";
			label = "${lbl:b_rel_num_control}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rel_fecha_registro");
			if (d != null)
			{

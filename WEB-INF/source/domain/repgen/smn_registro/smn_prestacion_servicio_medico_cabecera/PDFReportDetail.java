package domain.repgen.smn_registro.smn_prestacion_servicio_medico_cabecera;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("psc_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("psc_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_mov_caja_cabecera_id_name");
			if (d != null)
			{
			name  = "b_movement_caj";
			label = "${lbl:b_movement_caj}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_unidades_servicios_rf");
			if (d != null)
			{
			name  = "b_service_units";
			label = "${lbl:b_service_units}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_mov_caja_cabecera_id");
			if (d != null)
			{
			name  = "b_movement_caj";
			label = "${lbl:b_movement_caj}";

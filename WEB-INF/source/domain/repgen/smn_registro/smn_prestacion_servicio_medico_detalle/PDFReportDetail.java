package domain.repgen.smn_registro.smn_prestacion_servicio_medico_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("psd_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_mov_caja_detalle_id_name");
			if (d != null)
			{
			name  = "b_movement_caj_detail";
			label = "${lbl:b_movement_caj_detail}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("psd_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_mov_caja_detalle_id");
			if (d != null)
			{
			name  = "b_movement_caj_detail";
			label = "${lbl:b_movement_caj_detail}";
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_appointment_movement";
			label = "${lbl:b_appointment_movement}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

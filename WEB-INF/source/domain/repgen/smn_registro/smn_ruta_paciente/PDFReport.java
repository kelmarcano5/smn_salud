package domain.repgen.smn_registro.smn_ruta_paciente;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("rta_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("rta_ticket");
			if (d != null)
			{
			name  = "b_rta_ticket";
			label = "${lbl:b_rta_ticket}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rta_numero_documento_identidad");
			if (d != null)
			{
			name  = "b_rta_documento";
			label = "${lbl:b_rta_documento}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

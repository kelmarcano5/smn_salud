package domain.repgen.smn_registro.smn_afiliado_titular;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
			...
			if (d != null)
			{
			name  = "b_afi_documento";
			label = "${lbl:b_afi_documento}";
		...
		d = rs.getString("afi_numero_control");
			if (d != null)
			{
			name  = "b_num_control";
			label = "${lbl:b_num_control}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("afi_fecha_registro");
			if (d != null)
			{

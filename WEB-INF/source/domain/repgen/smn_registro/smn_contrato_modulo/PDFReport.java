package domain.repgen.smn_registro.smn_contrato_modulo;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_contrato_base_rf_name");
			if (d != null)
			{
			name  = "b_contrato_base";
			label = "${lbl:b_contrato_base}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_contrato_base_rf");
			if (d != null)
			{
			name  = "b_contrato_base";
			label = "${lbl:b_contrato_base}";

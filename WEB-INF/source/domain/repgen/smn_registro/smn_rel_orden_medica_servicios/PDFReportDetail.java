package domain.repgen.smn_registro.smn_rel_orden_medica_servicios;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_prestador_servicio_referido_rf_name");
			if (d != null)
			{
			name  = "b_service_providers";
			label = "${lbl:b_service_providers}";
		...
		d = rs.getString("smn_grupo_prestador_rf_name");
			if (d != null)
			{
			name  = "b_grupo_prestador_servicio";
			label = "${lbl:b_grupo_prestador_servicio}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_ordenes_medicas_id");
			if (d != null)
			{
			name  = "b_orders_medics";
			label = "${lbl:b_orders_medics}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_prestador_servicio_referido_rf");
			if (d != null)
			{
			name  = "b_service_providers";
			label = "${lbl:b_service_providers}";

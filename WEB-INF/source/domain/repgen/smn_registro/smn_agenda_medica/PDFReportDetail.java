package domain.repgen.smn_registro.smn_agenda_medica;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
			...
			if (d != null)
			{
			name  = "b_service_areas";
			label = "${lbl:b_service_areas}";
		...
		d = rs.getString("smn_agenda_id_name");
			if (d != null)
			{
			name  = "b_timetable";
			label = "${lbl:b_timetable}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("agm_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("agm_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";

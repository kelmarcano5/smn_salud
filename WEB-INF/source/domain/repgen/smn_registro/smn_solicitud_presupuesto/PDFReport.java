package domain.repgen.smn_registro.smn_solicitud_presupuesto;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_contratante_id_name");
			if (d != null)
			{
			name  = "b_contratante_aux";
			label = "${lbl:b_contratante_aux}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_patologia_id_name");
			if (d != null)
			{
			name  = "b_pathology";
			label = "${lbl:b_pathology}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_patologia_id");
			if (d != null)
			{
			name  = "b_pathology";
			label = "${lbl:b_pathology}";
		...
		d = rs.getString("spr_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("spr_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_auxiliar_name");
			if (d != null)
			{
			name  = "b_auxiliary";
			label = "${lbl:b_auxiliary}";

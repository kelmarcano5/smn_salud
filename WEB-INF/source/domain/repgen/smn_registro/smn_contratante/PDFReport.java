package domain.repgen.smn_registro.smn_contratante;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_documento_rf_name");
			if (d != null)
			{
			name  = "b_cnt_documento";
			label = "${lbl:b_cnt_documento}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cnt_numero_control");
			if (d != null)
			{
			name  = "b_cnt_num_control";
			label = "${lbl:b_cnt_num_control}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

package domain.repgen.smn_proceso.smn_recipe_medico_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_unidad_medida_rf");
			if (d != null)
			{
			name  = "b_measurement_unit";
			label = "${lbl:b_measurement_unit}";
		...
		d = rs.getString("smn_presentacion_rf_name");
			if (d != null)
			{
			name  = "b_presentacion";
			label = "${lbl:b_presentacion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("rmd_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_unidad_medida_rf_name");
			if (d != null)
			{
			name  = "b_measurement_unit";
			label = "${lbl:b_measurement_unit}";
		...
		d = rs.getString("rmd_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("hidden");
			if (d != null)
			{
			name  = "b_prescription_medic";
			label = "${lbl:b_prescription_medic}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_principio_activo_rf");
			if (d != null)
			{
			name  = "b_active_ingredient";
			label = "${lbl:b_active_ingredient}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_presentacion_rf");
			if (d != null)
			{
			name  = "b_presentacion";
			label = "${lbl:b_presentacion}";
		...
		d = rs.getString("rmd_tiempo_frecuencia");
			if (d != null)
			{
			name  = "b_frecuency_time";
			label = "${lbl:b_frecuency_time}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

package domain.repgen.smn_proceso.smn_plan_quirurgico;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_patologia_rf");
			if (d != null)
			{
			name  = "b_surgery";
			label = "${lbl:b_surgery}";
		...
		d = rs.getString("smn_contratante_id_name");
			if (d != null)
			{
			name  = "b_contratante";
			label = "${lbl:b_contratante}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("plq_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("plq_fecha_cirugia");
			if (d != null)
			{
			name  = "b_surgery_date";
			label = "${lbl:b_surgery_date}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("plq_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("smn_habitaciones_id");
			if (d != null)
			{
			name  = "b_habitaciones";
			label = "${lbl:b_habitaciones}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_patologia_rf_name");
			if (d != null)
			{
			name  = "b_surgery";
			label = "${lbl:b_surgery}";
		...
		d = rs.getString("smn_contratante_id");
			if (d != null)
			{
			name  = "b_contratante";
			label = "${lbl:b_contratante}";
		...
		d = rs.getString("smn_habitaciones_id_name");
			if (d != null)
			{
			name  = "b_habitaciones";
			label = "${lbl:b_habitaciones}";

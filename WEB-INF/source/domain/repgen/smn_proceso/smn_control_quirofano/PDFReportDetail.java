package domain.repgen.smn_proceso.smn_control_quirofano;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_ingreso_id_name");
			if (d != null)
			{
			name  = "b_ingreso";
			label = "${lbl:b_ingreso}";
		...
		d = rs.getString("smn_agenda_rf");
			if (d != null)
			{
			name  = "b_schedule";
			label = "${lbl:b_schedule}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ccq_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("ccq_fecha_quirofano");
			if (d != null)
			{
			name  = "b_fecha_quirofano";
			label = "${lbl:b_fecha_quirofano}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_agenda_rf_name");
			if (d != null)
			{
			name  = "b_schedule";
			label = "${lbl:b_schedule}";
		...
		d = rs.getString("ccq_fecha_registro_init");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";

package domain.repgen.smn_configuracion.smn_habitaciones;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("hab_tipo_servicio_name");
			if (d != null)
			{
			name  = "b_types_services";
			label = "${lbl:b_types_services}";
		...
		d = rs.getString("hab_tipo_servicio");
			if (d != null)
			{
			name  = "b_types_services";
			label = "${lbl:b_types_services}";
		...
		d = rs.getString("smn_activo_fijo_rf");
			if (d != null)
			{
			name  = "b_activo_fijo";
			label = "${lbl:b_activo_fijo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_activo_fijo_rf_name");
			if (d != null)
			{
			name  = "b_activo_fijo";
			label = "${lbl:b_activo_fijo}";
		...
		d = rs.getString("hab_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("hab_descripcion");
			if (d != null)
			{
			name  = "b_description";
		...
		d = rs.getString("hab_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";

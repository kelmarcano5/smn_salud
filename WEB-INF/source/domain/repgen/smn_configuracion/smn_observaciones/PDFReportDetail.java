package domain.repgen.smn_configuracion.smn_observaciones;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("obs_tipo");
			if (d != null)
			{
			name  = "b_tipo_obs";
			label = "${lbl:b_tipo_obs}";
		...
		d = rs.getString("obs_codigo");
			if (d != null)
			{
			name  = "b_code_obs";
			label = "${lbl:b_code_obs}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("obs_descripcion");
			if (d != null)
			{
			name  = "b_description_obs";
			label = "${lbl:b_description_obs}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

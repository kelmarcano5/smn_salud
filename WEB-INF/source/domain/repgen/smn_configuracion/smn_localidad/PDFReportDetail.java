package domain.repgen.smn_configuracion.smn_localidad;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("loc_descripcion");
			if (d != null)
			{
			name  = "b_descripcion_local";
			label = "${lbl:b_descripcion_local}";
		...
		d = rs.getString("loc_codigo");
			if (d != null)
			{
			name  = "b_codigo_local";
			label = "${lbl:b_codigo_local}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

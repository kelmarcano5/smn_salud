package domain.repgen.smn_configuracion.smn_tramos_descanso;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("trd_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("smn_tramos_id");
			if (d != null)
			{
			name  = "b_cod_tramo";
			label = "${lbl:b_cod_tramo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tramos_id_name");
			if (d != null)
			{
			name  = "b_cod_tramo";
			label = "${lbl:b_cod_tramo}";

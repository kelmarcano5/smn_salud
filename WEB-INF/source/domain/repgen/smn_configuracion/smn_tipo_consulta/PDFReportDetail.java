package domain.repgen.smn_configuracion.smn_tipo_consulta;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("tcs_fecha_registro");
			if (d != null)
			{
		...
		d = rs.getString("tcs_nombre");
			if (d != null)
			{
			name  = "b_tcs_nombre";
			label = "${lbl:b_tcs_nombre}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("tcs_codigo");
			if (d != null)
			{
			name  = "b_tcs_codigo";
			label = "${lbl:b_tcs_codigo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));

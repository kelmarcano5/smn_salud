package domain.repgen.smn_configuracion.smn_rel_documentos_servicios;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("rdt_tipo_generacion");
			if (d != null)
			{
			name  = "b_tipo_generacion";
			label = "${lbl:b_tipo_generacion}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_documentos_id_name");
			if (d != null)
			{
			name  = "b_contrato_doc";
			label = "${lbl:b_contrato_doc}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_documentos_id");
			if (d != null)
			{
			name  = "b_contrato_doc";
			label = "${lbl:b_contrato_doc}";

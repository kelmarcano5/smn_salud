...
public class Generarold extends GenericTransaction
{
	...
	public int service(Recordset inputParams) throws Throwable
	{
		...
		try {
						...
						if(rsCabeceraCaja.getRecordCount() <= 0){
											...
											if(rsDocumentoCaja.getRecordCount() > 0){
												...
												String sqlInsertCabeceraCaja = "INSERT INTO smn_caja.smn_mov_caja_cabecera( " +
																		        ...
																		        "VALUES (nextval('smn_caja.seq_smn_mov_caja_cabecera'), "+
																		            ...
																		            rsIngresos.getDouble("igs_monto_moneda_local")+","+
																		            rsIngresos.getInt("igs_monto_moneda_alterna")+", "+
							...
							/*String sqlInsertDetalleCaja = "INSERT INTO smn_caja.smn_mov_caja_detallee( " +
													      ...
													      " SELECT nextval('smn_caja.seq_smn_mov_caja_detalle'), "+rsCabeceraCaja.getInt("smn_mov_caja_cabecera_id")+", 'SS', " +
													            ...
													            "inm_monto_ma, inm_monto_ma, smn_moneda_rf, smn_tasa_rf, " + 
													            "'es', '"+inputParams.getString("usuario")+"','" +fechaActual+"', '"+ 
													            hora2+"' FROM smn_salud.smn_ingreso_movimiento WHERE not exists (SELECT * FROM smn_caja.smn_mov_caja_detalle WHERE smn_id_rf = smn_servicios_rf AND mcd_tipo_producto = 'SS' AND smn_mov_caja_cabecera_id = "+rsCabeceraCaja.getInt("smn_mov_caja_cabecera_id")+" AND mcd_fecha_registro = current_date)";*/
			...
			String fechaActual= new SimpleDateFormat("yyyy-MM-dd").format(new Date());	
			//Exportar log
			File newLogFile = new File("D:/data/Simone/smn_salud/log/log"+fechaActual+"_"+inputParams.getInt("id_ingreso")+".txt");
	...
	public int CallPrestationServices(Recordset inputParams) throws Throwable{
			...
			try {
				...
				File newLogFile = new File("D:/data/Simone/smn_salud/log/log_ProcesarPrestacionServicio"+fechaActual+"_"+inputParams.getInt("id_ingreso")+".txt");
				//File newLogFile = new File("E:/data/Simone/smn_salud/log/log_ProcesarPrestacionServicio"+fechaActual+"_"+inputParams.getInt("smn_mov_caja_cabecera_id")+".txt");

select
	(select smn_base.smn_usuarios.usr_usuario|| ' - ' || smn_base.smn_usuarios.usr_nombres from  smn_base.smn_usuarios where smn_base.smn_usuarios.smn_usuarios_id is not null  and smn_base.smn_usuarios.smn_usuarios_id=smn_salud.smn_rol.smn_usuario_id) as smn_usuario_id,
	case
	...
	(select smn_base.smn_corporaciones.crp_codigo|| ' - ' || smn_base.smn_corporaciones.crp_direccion from  smn_base.smn_corporaciones where smn_base.smn_corporaciones.smn_corporaciones_id is not null  and smn_base.smn_corporaciones.smn_corporaciones_id=smn_salud.smn_rol.smn_corporacion_rf) as smn_corporacion_rf,

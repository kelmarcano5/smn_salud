	...
	(select smn_base.smn_paso.pas_codigo || ' - ' ||  smn_base.smn_paso.pas_descripcion from  smn_base.smn_paso  where smn_base.smn_paso.smn_paso_id is not null  and smn_base.smn_paso.smn_paso_id=smn_salud.smn_relacion_unidad_paso.smn_paso_rf  order by smn_base.smn_paso.pas_descripcion ) as smn_paso_rf,
	(select smn_salud.smn_series.sri_codigo || ' - ' ||  smn_salud.smn_series.sri_nombre from  smn_salud.smn_series  where smn_salud.smn_series.smn_series_id is not null  and smn_salud.smn_series.smn_series_id=smn_salud.smn_relacion_unidad_paso.smn_serie_id  order by smn_salud.smn_series.sri_nombre ) as smn_serie_id,
	case

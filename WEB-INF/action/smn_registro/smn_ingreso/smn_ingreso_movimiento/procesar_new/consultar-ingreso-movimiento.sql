select
	...
	case
		when smn_base.smn_baremos.bar_tipo is null then 'G' 
		else smn_base.smn_baremos.bar_tipo end as tipo_precio, 
	case
		when smn_salud.smn_ingreso_movimiento.smn_moneda_rf is null then 0 
		else smn_salud.smn_ingreso_movimiento.smn_moneda_rf end as smn_moneda_rf,
	case 
		when smn_salud.smn_contratante.smn_baremos_id is null then (select 
																		smn_base.smn_baremos.smn_baremos_id 
																	from 
																		smn_base.smn_baremos 
																	where 
																		smn_base.smn_baremos.bar_tipo='G') 
		else smn_salud.smn_contratante.smn_baremos_id 
	end as baremos_id,
	smn_salud.smn_contratante.smn_contratante_id,
	case 
		when smn_salud.smn_ingresos.smn_contratante_id is null then smn_salud.smn_ingresos.smn_auxiliar_rf 
		else smn_salud.smn_contratante.smn_auxiliar_rf
	...
	case
		...
		else smn_salud.smn_ingreso_movimiento.smn_prestador_servicio_rf end
	as smn_prestador_servicio_rf,
	case
		when smn_salud.smn_ingreso_movimiento.smn_grupo_prestador_servicio_rf is null then 0 
		else smn_salud.smn_ingreso_movimiento.smn_grupo_prestador_servicio_rf end
...
where 
	...
	smn_salud.smn_ingreso_movimiento.smn_servicios_rf is not null and 
	smn_salud.smn_ingreso_movimiento.inm_estatus = 'AC'

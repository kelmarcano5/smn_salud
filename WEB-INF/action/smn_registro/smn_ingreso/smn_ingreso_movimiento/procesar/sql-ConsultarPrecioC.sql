SELECT smn_v_servicio_precio_id AS id_servicio,
	CASE
		WHEN smn_precio_neto_ml_servicio IS NOT NULL then smn_precio_neto_ma_servicio*(select smn_base.smn_tasas_de_cambio.tca_tasa_cambio 
			...
			where smn_base.smn_tasas_de_cambio.tca_fecha_vigencia=CURRENT_DATE 
...
WHERE smn_base.smn_v_servicio_precio.smn_tipo_precio = ${fld:tipo_precio} and smn_v_servicio_precio_id = ${fld:smn_servicios_rf} and smn_clase_auxiliar_rf=${fld:smn_clase_rf} AND smn_base.smn_tasas_de_cambio.tca_fecha_vigencia=CURRENT_DATE 
			and smn_base.smn_tasas_de_cambio.tca_tipo_tasa=${fld:tipo_tasa} 

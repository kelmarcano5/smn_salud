select distinct
	...
	smn_salud.smn_habitaciones.hab_descripcion as cubiculo,
	smn_base.smn_prestadores_servicios.prs_descripcion as medico,
	smn_salud.smn_sintoma.stm_descripcion as sintoma,
	smn_salud.smn_ingresos.smn_ingresos_id as ingreso_id
...
from
	...
	left join smn_salud.smn_habitaciones on smn_salud.smn_habitaciones.smn_habitaciones_id = smn_salud.smn_ingresos.smn_activo_fijo
	left join smn_base.smn_prestadores_servicios on smn_base.smn_prestadores_servicios.smn_prestadores_servicios_id = smn_salud.smn_prestacion_servicio_medico_cabecera.smn_prestador_servicio_rf
	inner join smn_salud.smn_sintoma on smn_salud.smn_sintoma.smn_sintoma_id = smn_salud.smn_ingresos.smn_sintoma_id
where
	 smn_base.smn_rel_servicio_area_unidad.smn_unidades_servicios_rf = ${fld:unidad_prestador} 
	 --and smn_salud.smn_prestacion_servicio_medico_cabecera.smn_grupo_prestador_rf = (select smn_base.smn_rel_grupo_prestador_servicio.smn_grupos_prestadores_id from smn_base.smn_rel_grupo_prestador_servicio 
--where smn_base.smn_rel_grupo_prestador_servicio.smn_prestadores_servicios_id=${fld:prestador} limit 1)
	 and smn_salud.smn_prestacion_servicio_medico_cabecera.psc_estatus='PE' and smn_salud.smn_ingresos.smn_activo_fijo is not null order by smn_salud.smn_ingresos.igs_ticket DESC

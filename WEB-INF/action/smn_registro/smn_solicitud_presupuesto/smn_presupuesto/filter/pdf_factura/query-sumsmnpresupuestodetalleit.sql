select SUM(smn_salud.smn_presupuesto_detalle.prd_monto_neto_ma) AS montomait
		--SUM(smn_salud.smn_presupuesto_detalle.prd_precio_ma) as precioma,
	    --SUM(smn_salud.smn_presupuesto_detalle.prd_moneda_alterna+smn_salud.smn_presupuesto_detalle.prd_monto_incremento_ma-smn_salud.smn_presupuesto_detalle.prd_monto_descuento_ma) as montomait,
	   --SUM(smn_salud.smn_presupuesto_detalle.prd_monto_descuento_ma) as descuentoma,
	   --SUM(smn_salud.smn_presupuesto_detalle.prd_monto_neto_ma) as netoma1
...
where 
	smn_salud.smn_presupuesto_detalle.smn_presupuesto_id=${fld:id} and smn_salud.smn_presupuesto_detalle.smn_tipo_componentes_rf in('IT') and smn_salud.smn_presupuesto_detalle.prd_estatus<>'RZ'
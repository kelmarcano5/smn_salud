 ...
 'SE' as smn_tipo_componentes_rf,
	...
	case
		...
		when smn_base.smn_componentes.cmp_forma_calculo='M' then 
			case
				when smn_base.smn_componentes.cmp_monto_moneda_local > 0 then smn_base.smn_servicios_componentes.sec_cantidad*cmp_monto_moneda_local
				else (smn_base.smn_servicios_componentes.sec_cantidad*cmp_monto_moneda_alterna * tca_tasa_cambio)
			end
	...
	case
		...
		when smn_base.smn_componentes.cmp_forma_calculo='M' then smn_base.smn_servicios_componentes.sec_cantidad*cmp_monto_moneda_alterna
	...
	smn_base.smn_modulos.mod_nombre ||' - '|| 'Componentes Generados desde Presupuesto Detalle' as pde_especificaciones_pedido,
	smn_salud.smn_presupuesto_detalle.prd_cantidad as prd_cantidad,
...
where
	smn_salud.smn_presupuesto.smn_presupuesto_id=${fld:smn_presupuesto_id} and smn_salud.smn_presupuesto_detalle.smn_presupuesto_detalle_id=${fld:smn_presupuesto_detalle_id} and smn_base.smn_servicios_componentes.smn_servicios_id=${fld:smn_servicios_rf}
ORDER BY

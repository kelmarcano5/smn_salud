select	
	...
	(select smn_base.smn_auxiliar.aux_num_doc_oficial || ' - ' || smn_base.smn_auxiliar.aux_nombres || ' ' || smn_base.smn_auxiliar.aux_apellidos from  smn_base.smn_auxiliar where smn_base.smn_auxiliar.smn_auxiliar_id is not null and smn_base.smn_auxiliar.smn_auxiliar_id = smn_salud.smn_presupuesto.smn_auxiliar_rf) as smn_auxiliar_rf_combo,
		(select smn_salud.smn_documento.doc_codigo|| ' - ' || smn_salud.smn_documento.doc_nombre from  smn_salud.smn_documento where smn_salud.smn_documento.smn_documento_id = smn_salud.smn_presupuesto.smn_documento_id ) as smn_documento_id_combo,
	...
	(select smn_base.smn_clase_auxiliar.cla_codigo|| ' - ' || smn_base.smn_clase_auxiliar.cla_nombre from  smn_base.smn_clase_auxiliar where smn_base.smn_clase_auxiliar.cla_codigo = 'PA' and smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id=smn_salud.smn_presupuesto.smn_clase_auxiliar_rf) as smn_clase_auxiliar_rf_combo,
	...
	(select smn_base.smn_clase_auxiliar.cla_codigo|| ' - ' || smn_base.smn_clase_auxiliar.cla_nombre from  smn_base.smn_clase_auxiliar where smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id is not null  and smn_base.smn_clase_auxiliar.smn_clase_auxiliar_id=smn_salud.smn_presupuesto.smn_clase_ord_rf) as smn_clase_ord_rf_combo,
	(select smn_base.smn_auxiliar_terceros.atr_codigo || ' - ' ||  smn_base.smn_auxiliar_terceros.atr_descripcion from smn_base.smn_auxiliar_terceros  where smn_base.smn_auxiliar_terceros.smn_auxiliar_terceros_id is not null  and smn_base.smn_auxiliar_terceros.smn_auxiliar_terceros_id = smn_salud.smn_presupuesto.smn_auxiliar_ord_rf) as smn_auxiliar_ord_rf_combo,
	(select smn_base.smn_baremos.bar_codigo || ' - ' ||  smn_base.smn_baremos.bar_descripcion from  smn_base.smn_baremos  where smn_base.smn_baremos.smn_baremos_id is not null  and smn_base.smn_baremos.smn_baremos_id=smn_salud.smn_presupuesto.smn_baremo_rf  order by smn_base.smn_baremos.bar_descripcion ) as smn_baremo_rf_combo,
	...
	(select smn_base.smn_monedas.mon_codigo || ' - ' || smn_base.smn_monedas.mon_nombre from  smn_base.smn_monedas  where smn_base.smn_monedas.smn_monedas_id is not null  and smn_base.smn_monedas.smn_monedas_id=smn_salud.smn_presupuesto.smn_moneda_rf  order by smn_base.smn_monedas.mon_nombre ) as smn_moneda_rf_combo,

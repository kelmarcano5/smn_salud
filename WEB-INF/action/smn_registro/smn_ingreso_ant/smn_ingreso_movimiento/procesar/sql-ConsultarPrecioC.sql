SELECT smn_v_servicio_precio_id AS id_servicio,
	CASE
		WHEN smn_moneda_alterna <> 0 then smn_precio_neto_ma_servicio*(select smn_base.smn_tasas_de_cambio.tca_tasa_cambio 
			...
			where smn_base.smn_tasas_de_cambio.tca_fecha_vigencia=(select smn_salud.smn_ingresos.igs_fecha_ingreso from smn_salud.smn_ingresos where smn_salud.smn_ingresos.smn_ingresos_id=${fld:smn_ingreso_id}) 
...
WHERE smn_base.smn_v_servicio_precio.smn_tipo_precio = ${fld:tipo_precio} and smn_v_servicio_precio_id = ${fld:smn_servicios_rf} and smn_clase_auxiliar_rf=${fld:smn_clase_rf} AND smn_base.smn_tasas_de_cambio.tca_fecha_vigencia=(select smn_salud.smn_ingresos.igs_fecha_ingreso from smn_salud.smn_ingresos where smn_salud.smn_ingresos.smn_ingresos_id=${fld:smn_ingreso_id}) 
			and smn_base.smn_tasas_de_cambio.tca_tipo_tasa=${fld:tipo_tasa} 

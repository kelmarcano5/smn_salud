select DISTINCT
	...
	case
		...
		when smn_salud.smn_ruta_paciente.rta_estatus='EA' THEN 'En Atencion'
...
where 
	  smn_ingresos_id is not null and smn_salud.smn_ruta_paciente.rta_estatus IN('EA','PE','RV') and 
	  smn_entidades_rf = ${fld:entidad_prestador} or 
	  smn_sucursales_rf = ${fld:sucursal_prestador} and
	  smn_areas_servicios_rf = ${fld:area_prestador} and
	  smn_unidades_servicios_rf = ${fld:unidad_prestador} and
	  igs_usuario = ${fld:usuario} and 
	  igs_fecha_registro = current_date --and smn_salud.smn_ingresos.igs_estatus_financiero NOT IN('PA') 
order by 
	smn_ingreso_id desc